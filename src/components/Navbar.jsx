...
//   return (
//     localStorage.getItem("studyNookUser") || localStorage.getItem("user") || ""
//   );
// };
...
//               <button
//                 onClick={logoutUser}
//                 className="w-full rounded-full border border-red-500/20 px-4 py-2 text-center font-semibold text-red-600 transition hover:bg-red-50 dark:hover:bg-red-500/10"
//                 type="button">
//                 Logout
//               </button>
//             )}
...
//   // Better-Auth এর রিয়েল-টাইম ডাটাবেজ সেশন ট্র্যাকিং হুক
//   const { data: session } = authClient.useSession();
//   const currentUser = session?.user;

//   // useSyncExternalStore দিয়ে গ্লোবাল থিম স্টেট ম্যানেজমেন্ট (এরর এড়ানোর বেস্ট উপায়)
//   const theme = useSyncExternalStore(
//     subscribeTheme,
//     getThemeSnapshot,
//     getServerThemeSnapshot,
...
//   // HTML ট্যাগে ক্লাস টগল করার জন্য শুধুমাত্র এই অংশটি কাজ করবে, কোনো setState নেই
//   useEffect(() => {
//     document.documentElement.classList.toggle("dark", darkMode);
//   }, [darkMode]);
...
//   // ডাটাবেজ থেকে সেশন ডিলিট করার লগআউট ফাংশন
...
//                 <button
//                   onClick={() => setOpen((prev) => !prev)}
//                   className="flex items-center gap-3 rounded-full border border-[#006B4F]/20 bg-white/80 p-1.5 pr-4 text-left shadow-xs backdrop-blur-md transition hover:bg-white dark:border-[#F6F0E4]/20 dark:bg-[#0f234f]/60 dark:hover:bg-[#0f234f]/90"
//                   type="button">
...
//                   <div className="flex flex-col justify-center leading-tight max-w-[120px]">
...
//                 {open && (
//                   <div className="absolute right-0 top-14 w-52 overflow-hidden rounded-2xl border border-[#006B4F]/15 bg-white/95 p-1.5 shadow-xl backdrop-blur-xl dark:border-[#F6F0E4]/15 dark:bg-[#07111f]">
//                     <Link
//                       href="/profile"
//                       onClick={() => setOpen(false)}
...
const Navbar = ({ handleLogout }) => {
  ...
  // Better-Auth এর রিয়েল-টাইম ডাটাবেজ সেশন ট্র্যাকিং হুক
  const { data: session } = authClient.useSession();
  const currentUser = session?.user;

  // useSyncExternalStore দিয়ে গ্লোবাল থিম স্টেট ম্যানেজমেন্ট (এরর এড়ানোর বেস্ট উপায়)
  const theme = useSyncExternalStore(
    subscribeTheme,
    getThemeSnapshot,
    getServerThemeSnapshot,
  ...
  // বাইরে ক্লিক করলে ডেস্কটপ ড্রপডাউন বন্ধ করার লজিক
  useEffect(() => {
        ...
        <nav className="relative mx-auto flex h-16 w-11/12 items-center justify-between rounded-full border border-[#006B4F]/20 bg-white/70 px-4 shadow-sm backdrop-blur-xl dark:border-[#F6F0E4]/20 dark:bg-[#07111f]/70 md:px-6">
          ...
          <div className="relative hidden shrink-0 items-center gap-4 md:flex">
            ...
            {!currentUser ? (
              ...
              // এই রেফটি ড্রপডাউন এবং বাটন দুইটাকেই কভার করছে
              <div className="relative" ref={dropdownRef}>
                ...
                {open && (
                  // top-[72px] দিয়ে বাটন থেকে প্রায় 8px নিচে গ্যাপ তৈরি করা হয়েছে
                  <div className="absolute right-0 top-[72px] w-52 overflow-hidden rounded-2xl border border-[#006B4F]/15 bg-white/95 p-1.5 shadow-xl backdrop-blur-xl dark:border-[#F6F0E4]/15 dark:bg-[#07111f]">
                    <Link
                      href="/profile"
                      onClick={() => setOpen(false)}
                      className="block rounded-xl px-4 py-2.5 text-xs font-bold text-[#4f4a40] transition hover:bg-[#006B4F]/10 hover:text-[#006B4F] dark:text-[#F6F0E4] dark:hover:bg-[#00D19A]/10 dark:hover:text-[#00D19A]">
