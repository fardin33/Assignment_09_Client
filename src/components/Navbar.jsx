...
//                 <Link
//                   href="/login"
//                   className={`flex-1 rounded-full border py-2.5 text-center text-xs font-bold transition ${pathname === "/login" ? "text-white" : "text-(--accent-green) dark:text-[#F6F0E4]"}`}
//                   style={
//                     pathname === "/login"
//                       ? { backgroundColor: "var(--accent-green)" }
//                       : { borderColor: "var(--accent-green)" }
    ...
    <>
      <header className="fixed top-0 left-0 right-0 z-50 bg-transparent pt-4">
        <nav className="relative mx-auto flex h-16 w-11/12 items-center justify-between rounded-full border border-[#006B4F]/20 bg-white/70 px-4 shadow-sm backdrop-blur-xl dark:border-[#F6F0E4]/20 dark:bg-[#07111f]/70 md:px-6">
          ...
          {/* Desktop Links (Now only visible on lg screen and up) */}
          ...
          {/* Desktop Right (Theme & Profile/Auth Buttons - Now only visible on lg screen and up) */}
          ...
          <button
            onClick={() => setMobileOpen((prev) => !prev)}
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[#006B4F]/25 bg-white/10 text-[#10231b] backdrop-blur-md transition-all duration-300 hover:bg-white/20 dark:border-[#F6F0E4]/25 dark:text-[#F6F0E4] lg:hidden"
            aria-label="Toggle menu"
            type="button">
            <span className="text-xl font-bold leading-none">
              {mobileOpen ? "×" : "☰"}
        ...
        {/* Mobile & Tablet Flyout Menu (Visible on screens smaller than lg) */}
        <div
          className={`absolute right-[4.5%] top-20 z-50 w-[91%] max-w-sm rounded-2xl border border-[#006B4F]/20 bg-white/95 px-4 py-5 shadow-2xl backdrop-blur-xl transition-all duration-300 ease-out dark:border-[#F6F0E4]/10 dark:bg-[#07111f] lg:hidden ${
            mobileOpen
              ? "translate-x-0 opacity-100"
              : "pointer-events-none translate-x-10 opacity-0"
          }`}>
